...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        # 初始化四个XCY模块
        # self.xcy1 = XCY1().to(self.device)
        # self.xcy2 = XCY2().to(self.device) 
        # self.xcy3 = XCY3().to(self.device) 
        # self.xcy4 = XCY4().to(self.device) 
    ...
    def __call__(self, preds, batch):
        ...
        # 分别调用四个XCY模块，并获取对应的L1正则化损失
        # l1_loss_weight = 0.5  # L1损失的权重
        # # #_, l1_loss1 = self.xcy1(feats[1])
        # # 在访问 feats[15] 之前，检查 feats 的长度
        # #print(f"feats 列表长度: {len(feats)}")  # 打印 feats 的长度
        # _, l1_loss2 = self.xcy2(feats[2])
        # _, l1_loss3 = self.xcy3(feats[1])
        # _, l1_loss4 = self.xcy4(feats[2])

        # #total_l1_loss = l1_loss_weight * (l1_loss1 + l1_loss2 + l1_loss3 + l1_loss4)
        # total_l1_loss = l1_loss_weight * (l1_loss2 + l1_loss3 + l1_loss4)
        # total_loss = loss.sum() * batch_size + total_l1_loss  # 将L1损失加到总损失中
