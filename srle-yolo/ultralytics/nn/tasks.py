...
from ultralytics.nn.Addmodules.HWD import HWD
import torch
import torch.nn as nn
from ultralytics.nn.biformer import BiFormerBlock
from ultralytics.nn.Addmodules.tssa import TSSA
from ultralytics.nn.Addmodules.tm import XCY
from ultralytics.nn.Addmodules.tm81 import XCY1
from ultralytics.nn.Addmodules.tm82 import XCY2
from ultralytics.nn.Addmodules.tm83 import XCY3
from ultralytics.nn.Addmodules.tm84 import XCY4
from ultralytics.nn.Addmodules.ASFYOLO import attention_model, Add, ScalSeq, Zoom_cat
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment, Detect_AFPN4, Detect_AFPN3, Detect_ASFF, Detect_FRM, Detect_dyhead, CLLAHead,
                          Detect_DySnakeConv, Detect_dyhead3, Detect_DySnakeConv, Segment_DBB, Detect_DBB, Detect_FASFF,
                          RFAHead, RFASegment, RepHead, Detect_Adown)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Segment, Pose, Detect_AFPN4, Detect_AFPN3, Detect_ASFF, Detect_FRM, Detect_dyhead,
                          CLLAHead, Detect_dyhead3, Detect_DySnakeConv, Segment_DySnakeConv, RepHead,
                          Segment_DBB, Detect_DBB, Pose_DBB, OBB, Detect_FASFF, RFAHead, RFAPose, RFASegment, Detect_Adown)):
            s = 640  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Segment_DySnakeConv, Pose, Pose_DBB, Segment_DBB,
                                                                     RFASegment, RFAPose, OBB)) else self.forward(x)
            try:
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 ...
                 RepNCSPELAN4_low,HWD,XCY,XCY1,XCY2,XCY3,XCY4):
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2f_ACmix, C2f_AKConv,
                     ...
                     CSPPC, CSPHet, C2f_Dual, C2f_DCNv3, Blocks, C2f_FasterBlock, C2f_DCNv3_DLKA):
        ...
        #---------------------------------------注意力机制------------------------
                   ...
                   deformable_LKA, FocalModulation, FocusedLinearAttention, HAT, iRMB, LSKA, TripletAttention, OREPA,
                   SCINet, Dy_Sample, CA, iAT, MultiSEAM, SEAM, deformable_LKA_Attention}:
            c2 = ch[f]
            args = [c2, *args]
        # ---------------------------------------注意力机制-----------------------
        # ---------------------------------------主干----------------------------
        ...
        # ------------------------------ASF-YOLO--------------------------------
        ...
        elif m is TSSA:#添加tssa
            c1, c2 = ch[f], args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, *args[1:]]
        elif m is BiFormerBlock:#添加的
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        elif m in (CBiF, BiFB):#添加的
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
              c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            if m in (CBiF, BiFB):
              ...
              n = 1
        #elif m is XCY:  # 当模型模块为XCY时
           # c1 = ch[f[-1]]  # 获取输入通道数（可根据具体需求调整）
            #height, width = args[1], args[2]  # 假设在YAML文件中，这些参数在args列表中传递
           # c2 = make_divisible(args[0] * width, 8)  # 可根据需要调整这个逻辑
            #args = [c1, c2, height, width, *args[3:]]  # 确保正确传递参数
        elif m is nn.BatchNorm2d:
            args = [ch[f]]
        elif m is ASFF2:#添加asff2、3
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, Segment_DySnakeConv, Segment_DBB, RFASegment)):
                return "segment"
            elif isinstance(m, Classify):
                return "classify"
            elif isinstance(m, (Pose, Pose_DBB, RFAPose)):
                return "pose"
