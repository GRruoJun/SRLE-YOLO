...
def tome_with_2d_distance(metric: torch.Tensor) -> Tuple[Callable, Callable]:
    ...
    # 定义相似度合并函数
    def merge_similarity(x: torch.Tensor, metric: torch.Tensor, mode="mean") -> torch.Tensor:
        t = metric.shape[1]  # 序列长度，metric (torch.Tensor): 输入的特征张量。
        r = t // 2
        metric = metric / metric.norm(dim=-1, keepdim=True)  # 归一化 metric
        a, b = metric[..., ::2, :], metric[..., 1::2, :]  # 将 metric 分为两个部分，交替取值
        ...
        def merge(x: torch.Tensor, mode="mean") -> torch.Tensor:  # 合并函数，将源和目标进行合并
            ...
            dst = dst.scatter_reduce(-2, dst_idx.expand(n, r, c), src, reduce=mode)  # 根据模式进行合并
...
class XCY(nn.Module):
    ...
    def forward(self, x):
        ...
        # 使用 tome_with_2d_distance 进行匹配
        merge_sim, merge_spatial = tome_with_2d_distance(x_flat)
        
        # 获取相似度和空间距离打分后的特征
        x_flat_sim1 = self._merge_and_process(x_flat, merge_sim)
        x_flat_sim2 = self._merge_and_process(x_flat_sim1, merge_sim)  # 调用两次 merge_sim
        
        x_flat_spatial = merge_spatial(x_flat)
        
        # 自适应融合
        x_flat_fused = self.fusion(x_flat_sim2, x_flat_spatial)
        
        # 计算并输出Kendall tau相关性
        kendall_tau_value = self.compute_kendall_tau(x_flat_sim2, x_flat_spatial)
        print("Kendall tau:", kendall_tau_value)
    ...
    def _merge_and_process(self, x_flat, merge_fn):
        """合并处理 x_flat 使用给定的合并函数"""
        return merge_fn(x_flat, x_flat)  # 将 x_flat 传递给 merge_fn 进行合并

    def compute_kendall_tau(self, similarity_scores, spatial_scores):
        n = similarity_scores.shape[0]
        tau_values = []

        for i in range(n):
            # 获取第 i 个 token 的相似度和空间距离分数
            sim_scores = similarity_scores[i].detach().cpu().numpy()  # 使用 .detach() 分离梯度
            spa_scores = spatial_scores[i].detach().cpu().numpy()     # 使用 .detach() 分离梯度

            # 计算 Kendall tau 秩相关系数
            tau, _ = kendalltau(sim_scores, spa_scores)
            tau_values.append(tau)

        # 返回平均 Kendall tau 值作为整体相关性
        return sum(tau_values) / (10*len(tau_values))
