import torch #13 在10的基础上把自适应融合改成了级联
...
class XCY(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv_reduce = nn.Conv2d(in_ch * 2, in_ch, kernel_size=1, stride=1)  # 用 1x1 卷积层来减小通道数
    def forward(self, x):
        B, C, H, W = x.shape
        ...
        # 在 C 维度上进行拼接，确保拼接时的维度是 B, C, H*W
        x_flat_sim2 = x_flat_sim2.transpose(1, 2).view(B, C, H // 2, W // 2)
        x_flat_spatial = x_flat_spatial.transpose(1, 2).view(B, C, H // 2, W // 2)

        # 在 C 维度拼接 (B, C, H, W)
        x_concat = torch.cat([x_flat_sim2, x_flat_spatial], dim=1)
        x_concat_reduced = self.conv_reduce(x_concat)  # 将通道数从 512 降为 256
   
        # 通过最终卷积层
        x = self.conv(x_concat_reduced)
