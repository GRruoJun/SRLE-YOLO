...
class AdaptiveFusion(nn.Module):
    def __init__(self, eps=1e-8):
        ...
        self.weights = nn.Parameter(torch.ones(3, dtype=torch.float32), requires_grad=True)  # 可学习权重（3个路径）
        self.eps = eps  # 避免除零错误

    def forward(self, similarity_scores, spatial_scores, avg_pool_scores):
        weights = nn.ReLU6()(self.weights)  # 使用ReLU6激活函数确保权重在0到6之间
        fuse_weights = weights / (torch.sum(weights) + self.eps)  # 归一化权重使它们的和为1
        return (fuse_weights[0] * similarity_scores +
                fuse_weights[1] * spatial_scores +
                fuse_weights[2] * avg_pool_scores)  # 加权融合相似度、空间距离和平均池化得分
...
class XCY(nn.Module):
    ...
    def forward(self, x):
        ...
        # 新增的平均池化路径
        x_flat_avg_pool = torch.mean(x_flat, dim=1, keepdim=True)  # 平均池化操作
        
        # 自适应融合
        x_flat_fused = self.fusion(x_flat_sim2, x_flat_spatial, x_flat_avg_pool)
    ...
    def _merge_and_process(self, x_flat, merge_fn):
        return merge_fn(x_flat, x_flat)  # 传递 metric 和 x_flat 进行合并处理
